...
radioList = {
    'nightwave': {
        ...
        'url': "http://radio.plaza.one/ogg",
        'youtube': False
    },
    'lofi': {
        'title': "Lofi Beats to chillout to.",
...
class VoiceState:
    ...
    @property
    def radio_mode(self):
        return self.voice and self.radio
...
class Music(commands.Cog):
    ...
    async def _radio(self, ctx: commands.context, *, radio: str):
        url = ""
        if not ctx.voice_state.voice:
            await ctx.invoke(self._join)
        ...
        if radio not in radioList.keys():
            return await ctx.send("""
                           The radio was not found.The following radios exist,
                           \t1. Nightwave 
                           \t2. LoFi
                           """)
        
        song = Radio(radio, ctx)
        ctx.voice_state.radio = song  
        ctx.voice_state.voice.play(song.audio)
        await song.channel.send(embed=song.create_embed())
